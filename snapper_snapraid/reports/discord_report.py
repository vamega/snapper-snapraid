...
def create_discord_report(report_data):
    ...
    if zero_subsecond_count > 0:
        touch_embed["description"] = (
            f"A total of **{zero_subsecond_count}** file(s) had their "
    ...
    if sync_job_ran:
        ...
        sync_embed["fields"] = [
            {"name": "Added", "value": f'```{diff_data["added"]}```', "inline": True},
            {
                "name": "Removed",
        ...
        sync_embed["footer"] = {"text": f"Elapsed time {sync_job_time}"}
    else:
    ...
    if scrub_job_ran:
        ...
        scrub_embed[
            "description"
        ] = f"""Scrub Job finished successfully :white_check_mark:
...
**{scrub_stats["unscrubbed"]}%** of the array has not been scrubbed, with the oldest block at **{scrub_stats["scrub_age"]}** day(s), the median at **{scrub_stats["median"]}** day(s), and the newest at **{scrub_stats["newest"]}** day(s)."""
        ...
        scrub_embed["footer"] = {"text": f"Elapsed time {scrub_job_time}"}
    else:
        scrub_embed["description"] = "Scrub job did **not** run."
